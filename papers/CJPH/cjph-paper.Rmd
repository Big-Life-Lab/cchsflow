...
The developers of *cchsflow* are public health researchers who collaborate with federal, provincial and local public health units. *cchsflow* began following publication of several peer-reviewed reports created with Public Health Ontario, ICES and the Ontario Public Health Association.[@ ] We faced several challenges. First, local Public Health Units requested additional analyses and updates which were difficult to support. Second, we used the same variable transformations in additional peer-review studies and reports. As projects evolved and team members changed, we found ourselves having difficulties keeping our variable code organized, consistent and visioned. Third, we translated our reports into on-line calculators at Project Big Life (https://projectbiglife.ca)[<https://projectbiglife.ca>] which required rewriting transformations in other web-enabled languages (JavaScript), survey tools (Lime Survey) and algorithm scoring engines (Predictive Modeling Mark-up Language).[@limesurvey2016][@Grossman1999] [@ProjectBigLife2020]

The variables in *cchsflow* are used peer-reviewed studies of our development team and other researchers.[@Manuel2012][\@Manuel2016][\@Manuel2018][\@Manuel2020] Occupation variables are an example that were incorporated from peer-reviewed occupation studies.[@nowrouzi2019] Depression variables are an example of variables where there were not consistent use in peer-review literature, but were added in consultation with mental health researchers. Open discusion with the mental health researchers is included in the package development. <https://github.com/Big-Life-Lab/cchsflow/pull/64> Anyone can participate in the discussions when new variables are added. One of us (DGM) is a part-time employee at Statistics Canada. The *cchsflow* package is not a Statistics Canada product, nor is the package endorsed by Statistics Canada. However, analysts at Statistics Canada use *cchslow* and have contributed to variable transformations.
...
As an open science addendum, you can use Rmarkdown to add the variables into the text to create dynamic reports that can be regularly and automatically updated. For example, we could have set the above code chunk to `echo = FALSE, results = hide` which hides the above code and then written the text as:
