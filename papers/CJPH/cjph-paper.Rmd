...
```{r Read variables.csv, message=FALSE, warning=FALSE}
library(readr)
...
cat("At time of writing, cchsflow contains: \n",
  ...
  sum(!duplicated(variables$subject)), "subjects \n", sum(!duplicated(variables$section)), "sections \n \n")

# Calculate the number of variables by section or subject

cat("The sections include: \n",
  sum(variables$section == "Health behaviour"), "health behaviour variables \n",
  sum(variables$section == "Sociodemographics"), "sociodemographic variables \n",
  sum(variables$section == "Health status"), "health stauts variables \n"
)
...
Sociodemographic variables include age, sex, immigration status, country of birth, time spent in Canada, ethnicity, education (individual and highest family), income (adjusted for province and inflation), home ownership, and marital status. A harmonized occupation variables were created  (`LBFA_31A`, `LBFA_31A_a` and `LBFA_31A_b`) by reviewing studies that used the CCHS to study occupation. References to these papers are included in the `notes` section of the variable transformation. 

Health status variables including chronic disease, the Health Utility Index According, need for help for activities of daily living, mental health, and other measures. The addition of mental health variables included a discussion with mental health researchers who regularly use the CCHS. Their feedback during the harmonization of these variables is open and available, where others can make further comments [https://github.com/Big-Life-Lab/cchsflow/pull/64)(https://github.com/Big-Life-Lab/cchsflow/pull/64). There is a new derived variables for the number of ADLs requiring assistance (`ADL_score_5`) that is not available in the original CCHS data. 
