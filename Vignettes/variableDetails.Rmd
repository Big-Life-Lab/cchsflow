...
### Rows
Each row in `variableDetails.csv` holds the recode rules for transforming a single category for a variable in `variables.csv`. An exception to this rule are the "don't know", "refusal", and "not stated" categories, which are combined as a single missing category. For each unique variable, an `else` row is used to assign values not identified in other rows. We recommend nor combining variables across the CCHS if variable has an important change between CCHS surveys. `variableDetails$notes` is used to identify issues that may be relevant when transforming the variable or category.  
...
8. **numValidCat:** the number of categories a variable has. This only applies to variables in which the **toType** is cat. For continuous variables, write `N/A`. Not applicable, missing, and else categories are not included in the category count. Note that this column is not necessary for `RecWTable`.
