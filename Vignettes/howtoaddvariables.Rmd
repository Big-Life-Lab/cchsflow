...
1. `variableDetails.csv`: This worksheet maps variables across CCHS cycles.
...
This example will show how the existing CCHS BMI variable was developed using `variableDetails.csv` and `variables.csv`. **Note** this variable is different from the derived BMI variable that is also included in _cchsflow_

### Specifying the variable on `variableDetails.csv`
...
kable(variableDetails[124:129, 1], col.names = 'variable')
```

2. **dummyVariable:** BMI is a continuous variable, so it does not have dummy variables.

```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:2])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:3])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[c(124, 129), 1:4])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[125:126, 1:4])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[127:128, 1:4])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[c(124, 129), 1:5])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[125:126, 1:5])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:6])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:9])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:10])
```

11. **units:** BMI is measured in kg/m^2^, so `kg/m2` is written in each row. 

```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:11])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, 1:12])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variableDetails[124:129, ])
```

### Specifying the variable on `variables.csv`

Once mapped and specified on `variableDetails.csv`, the BMI variable can now be specified on `variables.csv`
...
Along with specifying the variable on `variableDetails.csv` and `variables.csv`, a previous step is required in creating derived variables and that is creating a custom function that creates the derived variable from existing CCHS variables.
