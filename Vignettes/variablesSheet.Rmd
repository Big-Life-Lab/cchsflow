...
You can search for variables in the table below. Try searching for the three `age` variables that are used in the `Transform CCHS variables` vignette. All three variables are in the `age` subject. Try sorting the `subject` column by clicking the up chevron (&#9650;) beside the `subject` heading: the top three rows of the table should show the three `age` variables: `DHHGAGE_A`, `DHHGAGE_B` and `DHHGAGE_cont`.
