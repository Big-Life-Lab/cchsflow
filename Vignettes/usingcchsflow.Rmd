...
>>>>>>> c13084e2bcef32634748a59523b0caa617bc32a9
...
CCHS surveys include derived variables that have one additional harmonization considerations. Derived variables in CCHS are created from existing, original variables that are transformed into a new variable. An example is body mass index (BMI). Respondents are asked to report their height and weight. BMI is then dereived (calculated) using variables for height and weight. 

You harmonize CCHS derived variables using `recWTable()` and `variables.csv` just as you do for other CCHS `cchsflow` variables. The only additional consideration is dervived variables such as `BMI` usually require first harmonizing their underlying initial variables (height and weight). This first step of harmonizing underlying variables is completed for you but you must have the underlying variables in your dataset. For this reason, `cchsflow` includes variables such as height (`HWTGHTM`) and weight (`HWTGWTK`) -- these variables are the hamonized versions of height and weight.
...
At this time, the CCHS datasets cannot be shared publicly. But that does not mean you cannot use a saved CCHS dataset on your computer to transform variables. However, we have included a sample of 200 respondents in the CCHS 2003 and 2010 that you can use to try `cchsflow`.

```{r, eval=FALSE}
head(cchs2003)
```

## Transforming derived variables across cycles

Once specified on both `variables.csv` and `variableDetails.csv`, derived variables are transformed in the same manner as regular CCHS variables through `RecWTable()`. Since derived variables are based on transformed CCHS variables, if you want to only transform a derived variable, you will have to also transform its corresponding CCHS variables that it is derived from.

Using the BMI example illustrated in the [derivedVariables]('~../Vignettes/derivedVariables.html') article, `RecWTable()` is able to transform BMI across multiple cycles provided that height (`HWTGHTM`) and weight (`HWTGWTK`) are specified.
