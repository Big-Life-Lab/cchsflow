...
The **variable_details.csv** worksheet contain details for the variables in `variables.csv`. Information from `variable_details.csv` worksheet is used by the `rec_with_table()` function to transform variables identifed in `variable_details$variableStart` to the newly transformed variable in `variable_details$variable`. 
...
library(knitr)
library(kableExtra)
variable_details <- read.csv(file.path(getwd(), '../inst/extdata/variable_details.csv'))
...
kable(variable_details[c(75:79), 1], col.names = 'variable')
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:2)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:3)])
```

4. **databaseStart:** the CCHS surveys that contain the variable of interest, separated by commas. 

```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:4)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:5)])
```

+ The categorical `age` variable in the 2001 CCHS survey is `DHHAGAGE`. If the final variable name for categorical age in the **variable** column is `DHHGAGE`, you would write the following in this column: `cchs2001::DHHAGAGE`
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:6)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:7)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:10)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:11)])
```
...
The rules for each category of a new variable are a string in `recFrom` and value in `recTo`. These recode pairs are the same syntax as `{sjmisc::rec()`. Recode pairs are obtained from the RecFrom and RecTo columns
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:12)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:13)])
```

14. **variableStartShortLabel:** short form label describing the variable.

```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:14)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:15)])
```
...
```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(75:79), c(1:16)])
```
...
kable(sample_variable_details[1,], options = list(dom='t'))
```
