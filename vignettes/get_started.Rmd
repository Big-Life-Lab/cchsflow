...
## for Rusty: find <s>Waldo</s> labels!
```{r}
...
OK. I guess that I'll need to attach them. A bit of a pain if you ask me.

```{r}
sex2001 <- rec_with_table(cchs2001_p, "DHH_SEX", log = TRUE, var_labels = c(var_name = "DHH_SEX"))
```

There, that should do it! But wait, there still missing or I can't find them...
```{r}
get_label(sex2001)
```

Hmm, let me try to set it myself.
```{r}
set_label(sex2001$DHH_SEX, label = "Sex")
```
Looks good in the console message:
```
attr(,"label")
[1] Sex
```
...
Double check. What?!
```{r} 
get_label(sex2001$DHH_SEX)
```

Not sure what I am missing... Looks fine to me. https://strengejacke.github.io/sjlabelled/reference/set_label.html

I'll try a different way.
```{r} 
set_label(sex2001$DHH_SEX) <- "Sex"
get_label(sex2001$DHH_SEX)
```
OK. That works.
