...
The rules for the transformation in two dataframes that are included in _cchsflow_:
...
_cchsflow_ includes commonly used sociodemogrphaphic, health behaviour and health status variables.
...
3) Either not collected in all cycles or have major changes between cycles. These variable may be included in _cchsflow_ with a note that says which cycles include the variable and can be transformed to a common variable. See [Example 3](#example3) 
...
```{r}
cat("_cchsflow_ includes", length(unique(variable_details$notes)), "variables with notes.")
```
...
`rec_w_table` can include variable and value labels for the harmonized variables using the `var_labels` attribute.
...
Be carefull when use use `NA` in your study. See [`missing data (tagged_na)`](https://big-life-lab.github.io/cchsflow/articles/tagged_na_usage.html) for more information.
...
### Example 3. Transform a single variable from multiple databases that changes categories between cycles. {#example3}
...
Use the `set_data_labels()` to label the variables in your final dataset. Labels are lost during the database merging. `set_data_labels` set labels with the orginal information in `variables` and `variable_details`.
