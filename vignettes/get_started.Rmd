...
been a consistent name. _cchsflow_ uses the variable name from the 2007 cycle whenever 
...
The rules for the transformation and harmonization are in in two dataframes that are included in 
 `variables` and `variable_details`, both included in _cchsflow_. There are vignettes that further describe `variables` and `variable_details`, including how to add or customize transformed variables.
 
### Which variables are transformed? How "good" is the transformation?
 
_cchsflow_ includes commonly used sociodemogrphaphic, health behaviour and health status variables. In general, variables across CCHS cycles can be grouped into five categories:

1) Consistent and unchanged across all cycles. These variables have no changes to the wording of the question that was asked, the response values and who was asked the question (same inclusion criteria and skip pattern).

2) Mostly consistent across cycles. These variables may have a small change in wording or response category. Notes for these variables are printed to the console when they are recoded with `rec_with_table()`. The notes are contained in `variable_details$notes`. Caution is required when using these variables. The notes should be reviewed to assess whether these variables can be used in your study. See [Example 2](#example2).

3) Either not collected in all cycles or have major changes between cycles. These variable may be included in _cchsflow_ with a note that says which cycles include the variable and can be transformed to a common variable. See [Example 4](#example4). 

4) Mostly consistent but with complex changes or transformation rules. See the [Reference](../reference/index.html) for these derived and "Potentially problematic variables". See [Example 7](#example7).

5) Not consistent between cycles. Variable that are inconsistent or not asked in most cycles are not included in _cchsflow_. In the future, we hope to include these variables with an marker that they've been reviewed but rejected from inclusion in _cchsflow_. 
...
```{r}
warning("_cchsflow_ includes ", length(unique(variable_details$notes)), " variables with notes.")
```
...
`rec_with_table()` can include variable and value labels for the harmonized variables using the `var_labels` attribute. By default, `rec_with_table()` will include all labels if then all variables in `variables` are recoded. See [Example 6](#example6).
...
If you've been wathcing, my may have noticed that _cchsflow_ recodes the values in factor or categorical variables for 6 ("not applicable"), 7 ("don’t know"), and 8 ("refusal"), 9 ("not stated") into `tagged_NA` values NA(a) ("not applicable") and NA (b) ("missing"). There are two reasons. First, the typical uses of CCHS consider the two values "not applicable" and "missing" for most situations. Second, R and other open languages typically support only one NA. 

Be carefull when use use `NA` in your study. See [`missing data (tagged_na)`](../articles/tagged_na_usage.html)for more information.
...
This example shows how you can transform and combine a variable across multiple CCHS cycles. The sex variable in CCHS 2001 (`DHHA_SEX`) and CCHS 2012 (`DHH_SEX`) is transformed a common variable (`DHH_SEX`) and combined into a single dataset. In _cchsflow_ the CCHS 2007 variable name is used as the common variable name. See the [variable_details vignette](variable_details.html) for more information.
...
_cchsflow_ often includes a common harmonized categorical variable that has new, consistent categories across all cycles. These new categorical variables generally have fewer category levels than the original variables, reflecting the need to combine categories. Read the `Notes` variable for details.

## Label variables and categories

Use `set_data_labels()` to label the variables in your final dataset. Labels are lost during the database merging. `set_data_labels()` set labels with the orginal information in `variables` and `variable_details`.

```{r}
labeled_combined_cchs <- set_data_labels(data_to_label = combined_age_cont, variable_details = variable_details, variables_sheet = variables)
```
...
In the above example, `var_labels` is called in `rec_with_table()` to label the age and sex variables in the 2001 and 2012 datasets. Use `get_label()` to view the variable labels in your transformed dataset. As mentioned previously, `var_labels` can be used all the variables in `variables.csv` or a subset of variables. 
...
CCHS derived variables are recoded (harmonized) using the same method as other CCHS _cchsflow_ variables, with one additional considerations.
