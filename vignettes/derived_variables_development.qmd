...
This guide provides instructions for developing, testing, and implementing derived variables in cchsflow using modern tidyverse patterns. The approach emphasizes type safety, readable code, robust missing data handling, and comprehensive validation.
...
2. **Calculated (derived) variables** (aka 'derived variables') - creating new variables from mathematical functions or complex logic

This guide focuses on ** derived variables** that require custom R functions.

## Design principles

Our current approach follows these key principles:
...
- **Comprehensive missing data handling**: Standardized preprocessing that returns `haven::tagged()`.
- **Conventional R naming**: Functions follow tidyverse verb-first naming conventions.
- **Dual input handling**: Functions accept both raw CCHS data (6,7,8,9 codes) AND preprocessed `haven::tagged_na()` (for discussion).
- **Conventional Roxygen documentation**: Functions are documented using R community standard approach.
- **Versioning** - Fuction and test versioning is supported and encouraged. The same version approach for `variables.csv` and `variable_details.csv` is used within the @note field. 

## Function naming standards

All derived variable functions must follow R community naming conventions for consistency and discoverability.

### Standard verb patterns

```r
# PRIMARY VERBS for derived variables (simplified to 3 core verbs)
calculate_*()   # All computations and evaluations (BMI, drinking risk, smoking status)
categorize_*()  # Classification into groups (BMI categories, risk levels)  
score_*()       # Scoring systems (ADL scores, composite indices)

# HELPER VERBS for infrastructure (internal functions, not exported)
clean_*()       # Data preprocessing and validation
preprocess_*()  # CCHS missing code handling
validate_*()    # Parameter and input validation
generate_*()    # Helper utilities

# DEPRECATED VERBS (migrate to primary verbs)
assess_*()      # → Use calculate_*()
derive_*()      # → Use calculate_*()
classify_*()    # → Use categorize_*()
apply_*()       # → Use calculate_*()
```

### Naming convention rules

Follow this pattern: `{verb}_{health_domain}_{modifier}`

```r
# Examples of proper naming
calculate_bmi()                    # Simple calculation
calculate_drinking_risk_short()    # Evaluation with modifier
categorize_bmi()                   # Classification
score_adl()                        # Default scoring (5-item)
score_adl_6()                      # Modified scoring (6-item)
adjust_bmi()                       # Data adjustment/correction (legacy name)
```

### Function name migration (v3.0.0)

The following functions have been renamed to follow R conventions:
...
# ADL Functions  
adl_fun()           → calculate_adl()        # Evaluate ADL status
adl_score_5_fun()   → score_adl()            # Default 5-item
...
**Migration support**: Deprecated aliases are provided for backward compatibility during the transition period.

### Copy-paste functionality requirements

All derived variable functions must work correctly in three contexts:

1. **Scalar operations**: `calculate_bmi(1.75, 70)` - Single value calculations
2. **Vector operations**: `calculate_bmi(c(1.75, 1.80), c(70, 75))` - Multiple observations  
...
### Future development - comprehensive logging system

**Note**: Detailed logging and validation warnings are planned for a future version as part of the comprehensive recodeflow logging system. Current v3.0.0 functions include `log_level` parameters for forward compatibility but operate in silent mode.

```{r eval=FALSE}
# Current v3.0.0 behavior - all modes operate silently
calculate_bmi(height, weight) # Silent (default)
calculate_bmi(height, weight, log_level = "warning") # Silent (future: validation warnings)
calculate_bmi(height, weight, log_level = "verbose") # Silent (future: detailed messages)
```

### Future logging design goals

1. **Scalability**: Thousands of derivations won't produce warning spam
2. **Granular control**: Enable logging only where needed
3. **Development friendly**: Easy to debug specific functions
4. **Integration**: Unified with recodeflow logging infrastructure

## Generic helper functions

### Universal validation and preprocessing

All derived variable functions use generic helpers from `missing-data-functions.R`:

```{r eval=FALSE}
# Generic parameter validation (replaces stop() calls)
validate_parameter <- function(param_name, param_value, required = TRUE,
                               na_type = "b", log_level = "silent") {
  if (required && missing(param_value)) {
    if (log_level %in% c("warning", "verbose")) {
      warning(paste("Required parameter", param_name, "not provided"))
    }
...
# Generic preprocessing detection
...
# Unified preprocessing with pattern types
preprocess_cchs_missing_codes <- function(input_var,
                                          pattern_type = "standard_response",
                                          preserve_numeric = TRUE) {
  # Handles all CCHS missing data patterns with single function
}
```

### Benefits of generic helpers

1. **Consistency**: All functions use same validation logic
2. **Maintainability**: Fix bugs in one place
3. **No stops**: Functions return `haven::tagged_na()` instead of stopping
4. **Reusability**: Same helpers across all derived variables
5. **Versioning**: Internal functions have simple version tracking

## Hybrid validation architecture

### Constants + CSV integration

Supports both standalone users and `rec_with_table()` workflows:

```{r eval=FALSE}
# For standalone (copy-paste) users
BMI_VALIDATION_BOUNDS <- list(
  height = list(min = 0.82, max = 2.50),
  weight = list(min = 22.7, max = 209.1)
)

# Function uses constants as defaults
validate_height_bounds <- function(height,
                                   min_height = BMI_VALIDATION_BOUNDS$height$min,
                                   max_height = BMI_VALIDATION_BOUNDS$height$max, ...) {
  # Validation logic
}

# For rec_with_table() CSV integration
ANTHROPOMETRIC_BOUNDS <- list(
  HWTGHTM = list(min = 0.82, max = 2.50),
  HWTGWTK = list(min = 22.7, max = 209.1)
)

# Future: variable_details.csv integration
# Uses existing rec_with_table() range notation: "[0.82,2.50]"
validate_anthropometric_variable <- function(value, variable_name) {
  bounds <- ANTHROPOMETRIC_BOUNDS[[variable_name]]
  # Apply bounds from CSV when available
}
```

### Range notation in variable_details.csv

`rec_with_table()` already supports range notation like `[7,9]` for missing codes. This can be extended to validation bounds:

```csv
# Current pattern in variable_details.csv
variable,recStart,notes
ADL_01,"[7,9]","don't know (7); refusal (8); not stated (9)"

# Future pattern for validation bounds
variable,validation_range,notes
HWTGHTM,"[0.82,2.50]","Height validation bounds in meters"
HWTGWTK,"[22.7,209.1]","Weight validation bounds in kilograms"
```

## Internal function versioning

### Simple versioning for internal functions

Internal functions include lightweight versioning:

```{r eval=FALSE}
#' Validate height bounds with comprehensive checks
#'
#' @param height Numeric height value(s) to validate (in meters)
#' @param min_height Minimum valid height (default from BMI_VALIDATION_BOUNDS)
#' @param max_height Maximum valid height (default from BMI_VALIDATION_BOUNDS)
#' @param na_handling Character tag for NA handling ("a" or "b")
#' @param log_level Logging level: "silent" (default), "warning", "verbose"
#' @return Validated height or haven::tagged_na
#' @note Internal v1.0.0 (2025-01-02) - Created for BMI validation consistency
#' @keywords internal
validate_height_bounds <- function(...) {
  ...
}
```
...
1. **Internal functions**: Simple v1.0.0 format with creation date
2. **Public functions**: Full semantic versioning in @note
3. **Change tracking**: Brief description in @note
4. **Status tracking**: active, deprecated, experimental, legacy
...
Where possible, contants should be recorded in `variables.csv` and `variable_details.csv`.
```{r eval=FALSE}
...
calculate_bmi <- function(HWTGHTM, HWTGWTK,
                          ...
                          BMI_min = 15, BMI_max = 50) {
  # Auto-detect validation mode based on parameters
  validate_params <- !all(is.null(c(
...
test_that("complex case_when preserves tagged NA semantics", {
  ...
  mixed_result <- bmi_categorization_fun(
    c(1.75, haven::tagged_na("a"), 6),
...
#' derived_var_fun(
#'   var1 = c(1.5, 2.1), var2 = c(15.2, 25.8),
...
**rec_with_table() testing is MANDATORY** - This is the primary function of cchsflow and all derived variables must be tested with it.

#### Required rec_with_table() test pattern

Every derived variable function MUST include comprehensive rec_with_table() tests:

```r
test_that("function_name() integrates with rec_with_table() across all cycles", {
  # Test with multiple CCHS cycles
  cycles_to_test <- c("cchs2001_p", "cchs2005_p", "cchs2009_2010_p", 
                      "cchs2011_2012_p", "cchs2013_2014_p")
  
  for (cycle in cycles_to_test) {
    # Skip if cycle data not available in test environment
    if (!exists(cycle)) {
      skip(paste("Test data", cycle, "not available"))
    }
    
    # Test rec_with_table() integration
    result <- expect_silent(
      rec_with_table(get(cycle), c("input_var1", "input_var2", "derived_var_name"))
    )
    
    # Validate output structure
    expect_true("derived_var_name" %in% names(result))
    expect_true(nrow(result) > 0)
    
    # Test that function produces valid results
    valid_results <- sum(!is.na(result$derived_var_name))
    expect_gt(valid_results, 0, info = paste("No valid results in", cycle))
    
    # Test missing data handling
    missing_results <- sum(haven::is_tagged_na(result$derived_var_name))
    expect_gte(missing_results, 0, info = paste("Missing data handling failed in", cycle))
  }
})

test_that("function_name() maintains cross-cycle consistency", {
  # Test that same inputs produce same outputs across cycles
  
  # Create test scenarios
  test_scenarios <- list(
    valid_case = list(input1 = 1, input2 = 2, expected = 1),
    missing_case = list(input1 = 6, input2 = 2, expected = haven::tagged_na("a")),
    invalid_case = list(input1 = 99, input2 = 2, expected = haven::tagged_na("b"))
  )
  
  for (scenario_name in names(test_scenarios)) {
    scenario <- test_scenarios[[scenario_name]]
    
    # Test direct function call consistency
    result <- function_name(scenario$input1, scenario$input2)
    expect_equal(result, scenario$expected, 
                info = paste("Direct call failed for", scenario_name))
  }
})

test_that("function_name() performance with real data volumes", {
  # Test performance with realistic data volumes
  skip_if_not(exists("cchs2013_2014_p"), "Large test dataset not available")
  
  # Benchmark rec_with_table() performance
  start_time <- Sys.time()
  result <- rec_with_table(cchs2013_2014_p, 
                          c("input_var1", "input_var2", "derived_var_name"))
  end_time <- Sys.time()
  
  # Should complete within reasonable time (adjust threshold as needed)
  execution_time <- as.numeric(difftime(end_time, start_time, units = "secs"))
  expect_lt(execution_time, 30, "rec_with_table() performance too slow")
  
  # Validate output quality
  completion_rate <- sum(!is.na(result$derived_var_name)) / nrow(result)
  expect_gt(completion_rate, 0.1, "Too many missing results in real data")
})
```
...
- **Performance benchmarking**: Ensure acceptable speed with large datasets
...
# R/Tidyverse Documentation Standards

## Professional roxygen2 documentation requirements

All cchsflow derived variable functions must follow R community documentation standards. **Do NOT use custom formatting** (e.g., bold headings, non-standard sections).

### Standard roxygen2 pattern (REQUIRED)

```r
#' Descriptive function title (active voice, verb phrase)
#'
#' @description 
#' One paragraph describing what the function does. Focus on purpose and method.
#' Avoid unnecessary styling or custom formatting.
#'
#' @details
#' Additional implementation details, methodology notes, or important caveats.
#' Include transformation warnings here if needed.
#' 
#' **Transformation Warnings**:
#' ⚠️ **CAUTION - Breaking Changes**: Description of any breaking changes
#' ⚠️ **CAUTION - Population Restrictions**: Any restrictions on applicability
#'
#' @param param_name Description of parameter. Include valid values and data types.
#'   Use consistent formatting across all parameters.
#' @param param_name2 Second parameter description following same pattern.
#'
#' @return Description of return value including data type, possible values, and 
#'   missing data handling approach. Be specific about output structure.
#'
#' @examples
#' # Primary example using rec_with_table() (REQUIRED)
#' library(cchsflow)
...
#' function_name(c(1, 2, 6), c(2, 2, 7))
#'
#' @seealso 
#' \\code{\\link{related_function}} for related functionality
#'
#' @references
#' Academic citations with DOI when applicable:
#' Author, A. (Year). Title. Journal, Volume(Issue), pages. \\doi{10.xxxx/xxxxx}
#'
#' @note v3.0.0, last updated: YYYY-MM-DD, status: active, Note: Brief change description
#' @export
```

### Documentation anti-patterns (AVOID)

❌ **Custom formatting with bold headings**:
```r
#' @description 
#' **Purpose**: Custom formatting is non-standard
#' **Method**: Avoid this pattern
#' **Clinical Context**: Not following R conventions
```

❌ **Overly verbose descriptions**:
```r
#' @description 
#' This function creates a derived variable for the purpose of calculating
#' a very specific measure that does something complex with multiple steps
#' and various intermediate calculations... [continues for many lines]
```

❌ **Missing rec_with_table() examples**:
```r
#' @examples
...
✅ **Correct tidyverse-style documentation**:
```r
#' @description 
#' Calculate BMI from height and weight with comprehensive validation.
#'
#' @examples
#' # Primary usage with rec_with_table()
#' library(cchsflow)
#' result <- rec_with_table(cchs2013_2014_p, c("HWTGHTM", "HWTGWTK", "BMI_der"))
#' 
#' # Direct usage
...
The `@examples` section must prioritize cchsflow workflows:

1. **First example**: Always show `rec_with_table()` usage
2. **Second example**: Basic direct function usage
3. **Third example**: Vector processing or edge cases
4. **Fourth example**: Advanced usage or validation modes

```r
#' @examples
#' # Standard cchsflow workflow (REQUIRED FIRST)
#' library(cchsflow)
#' bmi_data <- rec_with_table(
#'   cchs2013_2014_p, 
#'   c("HWTGHTM", "HWTGWTK", "BMI_der")
#' )
#' head(bmi_data)
#' 
#' # Basic usage
...
Follow tidyverse conventions for parameter descriptions:

```r
#' @param HWTGHTM Height in meters. Accepts raw CCHS codes or preprocessed values.
#' @param HWTGWTK Weight in kilograms. Accepts raw CCHS codes or preprocessed values.
#' @param min_HWTGHTM Minimum valid height in meters (default 0.82).
#' @param validate_params Enable parameter validation (default NULL for auto-detection).
```

**Key principles**:
- Start with data type/content description
- Mention CCHS code compatibility
- Include defaults for optional parameters
- Use parallel structure across similar parameters

### Return value documentation

Be specific about output structure and missing data handling:
...
#'   }
#'   Valid range typically 15-50 for realistic measurements.
```
...
- [ ] Use standard roxygen2 pattern: @description, @details, @param, @return, @examples
- [ ] Lead @examples with rec_with_table() usage
- [ ] Include academic @references with DOI when applicable
- [ ] Use tidyverse-style parameter descriptions
- [ ] Specify missing data handling in @return

**Testing Requirements**:
- [ ] Add mandatory rec_with_table() integration tests across cycles
- [ ] Add comprehensive test coverage for direct function calls
- [ ] Include performance testing with real data volumes
- [ ] Test cross-cycle consistency

**Metadata**:
- [ ] Update @note metadata with v3.0.0 version format
- [ ] Ensure @export tag is present for public functions
...
complex_derived_fun <- function(var1, var2, var3, var4, ...) {
  ...
  list2env(
    setNames(processed_vars, c("var1", "var2", "var3", "var4")),
    environment()
  )
...
- **Missing data handling**: `R/missing-data-functions.R` - Universal preprocessing utilities
