...
Our current approach follows these key principles established through the v3.0.0 modernization:
...
```r
# Primary verbs for derived variables
calculate_*()   # Mathematical computations (BMI, energy expenditure, pack-years)
assess_*()      # Health risk evaluations (drinking risk, ADL needs, smoking status)
categorize_*()  # Classification into groups (BMI categories, risk levels)
score_*()       # Scoring systems (ADL scores, composite indices)
classify_*()    # Alternative to categorize for complex classification rules
```
