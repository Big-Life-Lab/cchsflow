...
#'
#' calculate_energy_expenditure(
#'   ages, paa_045, paa_050, paa_075, paa_080,
...
calculate_energy_expenditure <-
  function(DHHGAGE_cont, PAA_045, PAA_050, PAA_075, PAA_080, PAADVDYS,
    ...
    leisure_adult <-
      if_else2(
        DHHGAGE_cont >= 18 & !is.na(PAA_045) & !is.na(PAA_050) &
    ...
    leisure_youth <-
      if_else2(
        DHHGAGE_cont < 18 & !is.na(PAYDVTOA) & !is.na(PAYDVADL),
        ((PAYDVTOA) + (PAYDVADL)),
        if_else2(
          PAYDVTOA == "NA(a)" | PAYDVADL == "NA(a)",
          haven::tagged_na("a"), haven::tagged_na("b")
        )
      )
    ...
    physical_activity <-
      if_else2(
        !is.na(PAYDVVIG) & !is.na(leisure_youth) & !is.na(PAYDVDYS),
