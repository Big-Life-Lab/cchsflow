...
#'  cycles, height and weight must be 
#'  transformed and harmonized.
#' 
#' @note In earlier CCHS cycles (2001 and 2003), height was collected in inches;
#'  while in later CCHS
#'  cycles (2005+) it was collected in meters. To harmonize values across 
#'  cycles, height was converted to meters (to 3 decimal points). Weight was 
#'  collected in kilograms across all CCHS cycles, so no transformations were 
#'  required in the harmonization process.
#' 
...
#' 
#' library(cchsflow)
#' bmi2010 <- rec_with_table(data_source = cchs2010, variable_details = 
#' variableDetails, dataset_name = "cchs2010", variables = c("HWTGHTM", 
#' "HWTGWTK", "HWTGBMI_der"))
#' head(bmi2010)
#' 
#' bmi2012 <- rec_with_table(data_source = cchs2012, variable_details = 
#' variableDetails, dataset_name = "cchs2012", variables = c("HWTGHTM", 
#' "HWTGWTK", "HWTGBMI_der"))
#' tail(bmi2012)
#' 
