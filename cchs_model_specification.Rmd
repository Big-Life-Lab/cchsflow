---
title: "1 - Specifying the variables to be transformed"
date: 'Last updated: 2019-08-08'
...
## **Introduction**
The Model Specification Workbook (MSW) is a series of worksheets (CSV files) that describe different model components. The MSW is used to specify the model to transform and harmonize CCHS variables between surveys.


## **Two worksheets in the Model Specification Workbook**
1) `cchsVariableDetails` --- information on factors (categories) and how to transform final variables from their starting variables. 
2) `variablesCCHSFlow` --- information on transformed variables, including labels, section, variable type, and units.

## Creating the cchsVariableDetails worksheet
The **cchsVariableDetails** worksheet is the starting point in transforming variables. Information from this worksheet will be used by the `recode-with-table` function to harmonize CCHS variables between surveys. The `recode-with-table` function will use this worksheet to transform variable. The two components of this worksheets are the **rows** and the **columns.**

###Rows
Each unique variable will have its own row. Furthermore, each distinct category in a unique variable will have its own row. The only exception to this rule are the "don't know", "refusal", and "not stated" categories, which are lumped together as a single category named as missing. For each unique variable, an else row will also be added to account for improperly coded values.
...
* There should 4 rows, 1 for the continuous "category", 1 for not applicable, 1 for missing, and 1 for else
...
3. **databaseStart:** BMI was captured in all CCHS surveys between 2001 and 2014, so in each row, the CCHS identifers will be listed this column:
...
4. **variableStart:** In the 2001, 2003, and 2005 CCHS surveys the BMI variable differs from the common name, while in the CCHS surveys from 2007-2014, the BMI variable is the same as the common name. Therefore for each row, the variableStart column will look like this:
...
6. **recTo:** Since this is a continuous variable, the first row (the main "category") should have `copy` written. For the not applicable row, write `N/A`. For the missing and else rows, write `NA`.

7. **numValidCat:** Since this is a continuous variable, there are no actual categories; so write `N/A` in each row.

8. **catLabel:** For the first row, write `BMI`. Not applicable row write `not applicable`. Missing row: `missing`. Else row: `else`
...
11. **recFrom:** Going through the CCHS data documentation from 2001 to 2014, it was found that the lowest BMI value was 11.91 and the highest BMI value was 57.9. Therefore the recFrom for the first row should be written as `11.91:57.9`. In each CCHS survey, not applicable was coded as 999.96 so the recFrom for this row would be `999.96:999.96`. Similarly, in each CCHS survey don't know was coded as 999.97, refusal was coded as 999.98, and not stated was coded as 999.99. Therefore the recFrom for the missing row would be `999.97:999.99`. For the else row, just write `else`.
...
14. **notes: ** In the 2001 CCHS survey, this variable was restricted to participants aged 20-64. This can be noted in the first row. As well, don't know (999.97) and refusal (999.98) were not asked in this survey. This can be noted in the missing row.

## Creating the variablesCCHSFlow worksheet

The **variablesCCHSFlow** worksheet lists all the variables that have been used in the transformations between the CCHS surveys from 2001 to 2014. Each variable will have its own row. In each row there are 7 columns in this worksheet and they are as follows:
