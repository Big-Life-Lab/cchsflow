...
### Phase 4: Verification and Final Merge

After all thematic PRs are merged into dev:
...
2. Compare with original feature branch:
   ```bash
   # Check if any commits were missed
   git log feature/v3.0.0-validation-infrastructure --not dev \
     -- inst/extdata/variables.csv inst/extdata/variable_details.csv
   ```

3. Create a summary document listing all merged changes.

## Implementation Timeline

Estimated timeline for implementation:

- **Phase 1** (Formatting): 1 day
  - Apply standardization script: 1 hour
  - PR review and merge: 4-8 hours

- **Phase 2** (Structure): 2-3 days
  - Create restructuring script: 2-4 hours
  - Verify changes: 2 hours
  - PR review and merge: 1-2 days

- **Phase 3** (Content): 1-2 weeks
  - Group 1 (Infrastructure): 2-3 days
  - Group 2 (BMI): 1-2 days
  - Group 3 (Smoking): 2-3 days
  - Group 4 (ADL): 1 day
  - Group 5 (Oral Health): 1-2 days
  - Group 6 (Other): 1 day

- **Phase 4** (Verification): 1-2 days

**Total**: 2-3 weeks with proper review time
...
1. **Merge Conflicts**
   - **Risk**: Cherry-picking may cause conflicts if other changes were
     merged to dev
   - **Mitigation**: Process in order, resolve conflicts carefully, verify
     with diff
   - **Solution**: If conflicts are complex, consider squash merging groups

2. **Lost Changes**
   - **Risk**: Some changes might be missed during cherry-picking
   - **Mitigation**: Track all commits in a spreadsheet, verify after each
     phase
   - **Solution**: Use `git log --not` to find missing commits

3. **Formatting Inconsistency**
   - **Risk**: Manual edits might reintroduce formatting issues
   - **Mitigation**: Always run `quote-empty-cells.py` after cherry-picking
   - **Solution**: Add pre-commit hook to enforce formatting

4. **Data Corruption**
   - **Risk**: Restructuring might corrupt CSV data
   - **Mitigation**: Verify row counts after each step, test with pandas
   - **Solution**: Keep backups, have rollback plan

5. **Review Fatigue**
   - **Risk**: Reviewers may get fatigued with multiple PRs
   - **Mitigation**: Space out PRs, provide clear documentation
   - **Solution**: Automate verification where possible

## Tools and Scripts

### Required Files

1. **quote-empty-cells.py** (existing)
   - Standardizes CSV formatting
   - Usage: `python3 quote-empty-cells.py`

2. **restructure_csvs.py** (to be created)
   - Adds new columns and reorders existing ones
   - Preserves existing data
   - Usage: `python3 restructure_csvs.py`

3. **verify_csv_integrity.py** (recommended to create)
   ```python
   import pandas as pd
   import sys

   def verify_csv(file_path, expected_rows, expected_cols):
   ...
   # Usage
   vars_ok = verify_csv('inst/extdata/variables.csv', 379, 16)
   details_ok = verify_csv('inst/extdata/variable_details.csv', 3721, 22)

   sys.exit(0 if vars_ok and details_ok else 1)
   ```

### Git Helpers

Add these git aliases to make the process easier:

```bash
git config alias.csv-commits "log --oneline \
  feature/v3.0.0-validation-infrastructure --not dev -- \
  inst/extdata/variables.csv inst/extdata/variable_details.csv"

git config alias.csv-diff "diff --stat dev..HEAD -- \
  inst/extdata/variables.csv inst/extdata/variable_details.csv"

git config alias.csv-verify "!wc -l inst/extdata/variables.csv \
...
1. ✅ All 33 commits from feature branch are accounted for
