...
test_test("adjusted_bmi_fun() handles one or more NA arguments properly",{
  expect_equal(adjusted_bmi_fun(NA, 1.75, 75), tagged_na("b"))
  expect_equal(adjusted_bmi_fun(1, NA, 75), tagged_na("b"))
  expect_equal(adjusted_bmi_fun(1, 1.75, NA), tagged_na("b"))
  expect_equal(adjusted_bmi_fun(NA, NA, NA), tagged_na("b"))
})

test_that("adjusted_bmi_fun() generates a bmi when appropriate parameters
          are passed", {
  expect_equal(adjusted_bmi_fun(1, 1.75, 75), 25.27331)
})
