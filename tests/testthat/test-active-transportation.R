...
test_that("active_transport2_fun() has expected outputs when
          all parameters are in range", {
  expect_equal(
    active_transport2_fun(1, 1, 45, 1, 1, 45),
    1
  )
})
...
test_that("active_transport2_fun() has expected outputs when
  ...
  expect_equal(
    active_transport2_fun(NA, NA, NA, 1, 1, 90),
    1
  )
})
...
test_that("active_transport2_fun() has expected outputs when
  ...
  expect_equal(
    active_transport2_fun(1, 1, 90, NA, NA, NA),
    1
  )
})
...
test_that("active_transport2_fun() has expected outputs when
  ...
  expect_equal(
    active_transport2_fun(NA, 1, 1, NA, 1, 1),
    tagged_na("b")
  )
})
